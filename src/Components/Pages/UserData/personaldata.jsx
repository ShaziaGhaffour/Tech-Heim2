import React, { useState } from 'react';
import { useNavigate } from 'react-router';
import ProfileContent from '../../Pages/ProfileContent';
import PaymentContent from '../../Pages/PaymentContent';
import Orders from '../../Pages/Orders';
import Wishlist from '../../Pages//Wishlist';
import Discount from '../../Pages/Discount';
import Security from '../../Pages/Security';
import LogoutModel from '../../Pages/LogoutModel';
import SignupForm from '../../Pages/SignUP/signup';
import Footer from '../../HomePagecomponents/Footer/Footer';

const Profile = () => {

    const [showModel, setShowModel] = useState(false);
    const closeModel = () => {
        setShowModel(false);
    }

    const [showLoginModel, setShowLoginModel] = useState(false);

    const handlecloseLogin = () => {
        setShowLoginModel(false);
    }


    const [activeTab, setActiveTab] = useState("personal");

    const renderActiveContent = () => {
        switch (activeTab) {
            case 'personal':
                return <ProfileContent />;
            case "payment":
                return <PaymentContent />;
            case "orders":
                return <Orders />
            case "wishlist":
                return <Wishlist />
            case "discount":
                return <Discount />
            case "security":
                return <Security />
            default:
                return null;
        }
    }

    const navigate = useNavigate();
    const handleContactUsClick = () => {
        navigate("/contactUs");
    }

    return (
        <>
            <div className='profile-content-container'>
                <div className="account-lists">
                    <div className="account-content">
                        <svg className='profile-svg' width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M30.3002 33.8249C30.2502 33.8249 30.1752 33.8249 30.1252 33.8249C30.0502 33.8249 29.9502 33.8249 29.8752 33.8249C24.2002 33.6499 19.9502 29.225 19.9502 23.775C19.9502 18.225 24.4752 13.7 30.0252 13.7C35.5752 13.7 40.1002 18.225 40.1002 23.775C40.0752 29.25 35.8002 33.6499 30.3752 33.8249C30.3252 33.8249 30.3252 33.8249 30.3002 33.8249ZM30.0002 17.425C26.5002 17.425 23.6752 20.275 23.6752 23.75C23.6752 27.1749 26.3502 29.9499 29.7502 30.0749C29.8252 30.0499 30.0752 30.0499 30.3252 30.0749C33.6752 29.8999 36.3002 27.15 36.3252 23.75C36.3252 20.275 33.5002 17.425 30.0002 17.425Z"
                                fill="#B4B4B4" />
                            <path
                                d="M30.0001 56.875C23.2751 56.875 16.8501 54.375 11.8751 49.825C11.4251 49.425 11.2251 48.825 11.2751 48.25C11.6001 45.275 13.4501 42.5 16.5251 40.45C23.9751 35.5 36.0501 35.5 43.4751 40.45C46.5501 42.525 48.4001 45.275 48.7251 48.25C48.8001 48.85 48.5751 49.425 48.1251 49.825C43.1501 54.375 36.7251 56.875 30.0001 56.875ZM15.2001 47.75C19.3501 51.225 24.5751 53.125 30.0001 53.125C35.4251 53.125 40.6501 51.225 44.8001 47.75C44.3501 46.225 43.1501 44.75 41.3751 43.55C35.2251 39.45 24.8001 39.45 18.6001 43.55C16.8251 44.75 15.6501 46.225 15.2001 47.75Z"
                                fill="#B4B4B4" />
                            <path
                                d="M30 56.875C15.175 56.875 3.125 44.825 3.125 30C3.125 15.175 15.175 3.125 30 3.125C44.825 3.125 56.875 15.175 56.875 30C56.875 44.825 44.825 56.875 30 56.875ZM30 6.875C17.25 6.875 6.875 17.25 6.875 30C6.875 42.75 17.25 53.125 30 53.125C42.75 53.125 53.125 42.75 53.125 30C53.125 17.25 42.75 6.875 30 6.875Z"
                                fill="#B4B4B4" />
                        </svg>
                        <div className="heading">Jimmy smith</div>
                    </div>

                    <div className={`account-content ${activeTab === 'personal' ? 'active-tab' : ''}`}
                        onClick={() => setActiveTab("personal")}>

                        <svg className="account-content-svg" width="24" height="24" viewBox="0 0 24 24" fill="currentcolor"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M12 12.75C8.83 12.75 6.25 10.17 6.25 7C6.25 3.83 8.83 1.25 12 1.25C15.17 1.25 17.75 3.83 17.75 7C17.75 10.17 15.17 12.75 12 12.75ZM12 2.75C9.66 2.75 7.75 4.66 7.75 7C7.75 9.34 9.66 11.25 12 11.25C14.34 11.25 16.25 9.34 16.25 7C16.25 4.66 14.34 2.75 12 2.75Z"
                                fill="#444444" />
                            <path
                                d="M15.8201 22.7499C15.4401 22.7499 15.0801 22.6099 14.8201 22.3499C14.5101 22.0399 14.3701 21.5899 14.4401 21.1199L14.6301 19.7699C14.6801 19.4199 14.8901 19.0099 15.1401 18.7499L18.68 15.2099C20.1 13.7899 21.3501 14.5999 21.9601 15.2099C22.4801 15.7299 22.7501 16.2899 22.7501 16.8499C22.7501 17.4199 22.4901 17.9499 21.9601 18.4799L18.42 22.0199C18.17 22.2699 17.7501 22.4799 17.4001 22.5299L16.05 22.7198C15.97 22.7398 15.9001 22.7499 15.8201 22.7499ZM20.31 15.9199C20.13 15.9199 19.97 16.0399 19.74 16.2699L16.2001 19.8099C16.1701 19.8399 16.12 19.9399 16.12 19.9799L15.9401 21.2299L17.1901 21.0499C17.2301 21.0399 17.33 20.9899 17.36 20.9599L20.9001 17.4199C21.0601 17.2599 21.2501 17.0299 21.2501 16.8499C21.2501 16.6999 21.1301 16.4899 20.9001 16.2699C20.6601 16.0299 20.48 15.9199 20.31 15.9199Z"
                                fill="#444444" />
                            <path
                                d="M20.9201 19.2198C20.8501 19.2198 20.7801 19.2099 20.7201 19.1899C19.4001 18.8199 18.3501 17.7699 17.9801 16.4499C17.8701 16.0499 18.1001 15.6399 18.5001 15.5299C18.9001 15.4199 19.3101 15.6499 19.4201 16.0499C19.6501 16.8699 20.3001 17.5199 21.1201 17.7499C21.5201 17.8599 21.7501 18.2799 21.6401 18.6699C21.5501 18.9999 21.2501 19.2198 20.9201 19.2198Z"
                                fill="#444444" />
                            <path
                                d="M3.41016 22.75C3.00016 22.75 2.66016 22.41 2.66016 22C2.66016 17.73 6.85018 14.25 12.0002 14.25C13.0902 14.25 14.1702 14.41 15.1802 14.71C15.5802 14.83 15.8002 15.25 15.6802 15.64C15.5602 16.04 15.1402 16.26 14.7502 16.14C13.8702 15.88 12.9502 15.74 12.0002 15.74C7.68018 15.74 4.16016 18.54 4.16016 21.99C4.16016 22.41 3.82016 22.75 3.41016 22.75Z"
                                fill="#444444" />
                        </svg>

                        <div className='sub-heading-content'>
                            <div className="sub-heading personal-data">Personal Data</div>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.7397 16.2799C10.5497 16.2799 10.3597 16.2099 10.2097 16.0599C9.91969 15.7699 9.91969 15.2899 10.2097 14.9999L13.2097 11.9999L10.2097 8.99991C9.91969 8.70991 9.91969 8.22991 10.2097 7.93991C10.4997 7.64991 10.9797 7.64991 11.2697 7.93991L14.7997 11.4699C15.0897 11.7599 15.0897 12.2399 14.7997 12.5299L11.2697 16.0599C11.1197 16.2099 10.9297 16.2799 10.7397 16.2799Z" fill="#B4B4B4" />
                            </svg>
                        </div>
                    </div>

                    <div className={`account-content ${activeTab === "payment" ? 'active-tab' : ''}`}
                        onClick={() => setActiveTab("payment")}>
                        <svg className="account-content-svg" width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M13.9002 17.4201H11.3902C9.75016 17.4201 8.42016 16.0401 8.42016 14.3401C8.42016 13.9301 8.76016 13.5901 9.17016 13.5901C9.58016 13.5901 9.92016 13.9301 9.92016 14.3401C9.92016 15.2101 10.5802 15.9201 11.3902 15.9201H13.9002C14.5502 15.9201 15.0902 15.3401 15.0902 14.6401C15.0902 13.7701 14.7802 13.6001 14.2702 13.4201L10.2402 12.0001C9.46016 11.7301 8.41016 11.1501 8.41016 9.36008C8.41016 7.82008 9.62016 6.58008 11.1002 6.58008H13.6102C15.2502 6.58008 16.5802 7.96008 16.5802 9.66008C16.5802 10.0701 16.2402 10.4101 15.8302 10.4101C15.4202 10.4101 15.0802 10.0701 15.0802 9.66008C15.0802 8.79008 14.4202 8.08008 13.6102 8.08008H11.1002C10.4502 8.08008 9.91016 8.66008 9.91016 9.36008C9.91016 10.2301 10.2202 10.4001 10.7302 10.5801L14.7602 12.0001C15.5402 12.2701 16.5902 12.8501 16.5902 14.6401C16.5802 16.1701 15.3802 17.4201 13.9002 17.4201Z" fill="#0C0C0C" />
                            <path d="M12.5 18.75C12.09 18.75 11.75 18.41 11.75 18V6C11.75 5.59 12.09 5.25 12.5 5.25C12.91 5.25 13.25 5.59 13.25 6V18C13.25 18.41 12.91 18.75 12.5 18.75Z" fill="#0C0C0C" />
                            <path d="M12.5 22.75C6.57 22.75 1.75 17.93 1.75 12C1.75 6.07 6.57 1.25 12.5 1.25C18.43 1.25 23.25 6.07 23.25 12C23.25 17.93 18.43 22.75 12.5 22.75ZM12.5 2.75C7.4 2.75 3.25 6.9 3.25 12C3.25 17.1 7.4 21.25 12.5 21.25C17.6 21.25 21.75 17.1 21.75 12C21.75 6.9 17.6 2.75 12.5 2.75Z" fill="#0C0C0C" />
                        </svg>
                        <div className='sub-heading-content'>
                            <div className="sub-heading personal-data"> Payment & Instalments</div>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.7397 16.2799C10.5497 16.2799 10.3597 16.2099 10.2097 16.0599C9.91969 15.7699 9.91969 15.2899 10.2097 14.9999L13.2097 11.9999L10.2097 8.99991C9.91969 8.70991 9.91969 8.22991 10.2097 7.93991C10.4997 7.64991 10.9797 7.64991 11.2697 7.93991L14.7997 11.4699C15.0897 11.7599 15.0897 12.2399 14.7997 12.5299L11.2697 16.0599C11.1197 16.2099 10.9297 16.2799 10.7397 16.2799Z" fill="#B4B4B4" />
                            </svg>
                        </div>
                    </div>

                    <div className={`account-content ${activeTab === 'orders' ? 'active-tab' : ''}`}
                        onClick={() => setActiveTab("orders")}>

                        <svg className="account-content-svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M5.18988 6.37994C4.99988 6.37994 4.79988 6.29994 4.65988 6.15994C4.36988 5.86994 4.36988 5.38994 4.65988 5.09994L8.28988 1.46994C8.57988 1.17994 9.05988 1.17994 9.34988 1.46994C9.63988 1.75994 9.63988 2.23994 9.34988 2.52994L5.71988 6.15994C5.56988 6.29994 5.37988 6.37994 5.18988 6.37994Z"
                                fill="#444444" />
                            <path
                                d="M18.8101 6.37994C18.6201 6.37994 18.4301 6.30994 18.2801 6.15994L14.6501 2.52994C14.3601 2.23994 14.3601 1.75994 14.6501 1.46994C14.9401 1.17994 15.4201 1.17994 15.7101 1.46994L19.3401 5.09994C19.6301 5.38994 19.6301 5.86994 19.3401 6.15994C19.2001 6.29994 19.0001 6.37994 18.8101 6.37994Z"
                                fill="#444444" />
                            <path
                                d="M20.21 10.6001C20.14 10.6001 20.07 10.6001 20 10.6001H19.77H4C3.3 10.6101 2.5 10.6101 1.92 10.0301C1.46 9.5801 1.25 8.8801 1.25 7.8501C1.25 5.1001 3.26 5.1001 4.22 5.1001H19.78C20.74 5.1001 22.75 5.1001 22.75 7.8501C22.75 8.8901 22.54 9.5801 22.08 10.0301C21.56 10.5501 20.86 10.6001 20.21 10.6001ZM4.22 9.1001H20.01C20.46 9.1101 20.88 9.1101 21.02 8.9701C21.09 8.9001 21.24 8.6601 21.24 7.8501C21.24 6.7201 20.96 6.6001 19.77 6.6001H4.22C3.03 6.6001 2.75 6.7201 2.75 7.8501C2.75 8.6601 2.91 8.9001 2.97 8.9701C3.11 9.1001 3.54 9.1001 3.98 9.1001H4.22Z"
                                fill="#444444" />
                            <path
                                d="M9.75977 18.3C9.34977 18.3 9.00977 17.96 9.00977 17.55V14C9.00977 13.59 9.34977 13.25 9.75977 13.25C10.1698 13.25 10.5098 13.59 10.5098 14V17.55C10.5098 17.97 10.1698 18.3 9.75977 18.3Z"
                                fill="#444444" />
                            <path
                                d="M14.3599 18.3C13.9499 18.3 13.6099 17.96 13.6099 17.55V14C13.6099 13.59 13.9499 13.25 14.3599 13.25C14.7699 13.25 15.1099 13.59 15.1099 14V17.55C15.1099 17.97 14.7699 18.3 14.3599 18.3Z"
                                fill="#0C0C0C" />
                            <path
                                d="M14.8902 22.75H8.86024C5.28024 22.75 4.48024 20.62 4.17024 18.77L2.76024 10.12C2.69024 9.71 2.97024 9.33 3.38024 9.26C3.79024 9.19 4.17024 9.47 4.24024 9.88L5.65024 18.52C5.94024 20.29 6.54024 21.25 8.86024 21.25H14.8902C17.4602 21.25 17.7502 20.35 18.0802 18.61L19.7602 9.86C19.8402 9.45 20.2302 9.18 20.6402 9.27C21.0502 9.35 21.3102 9.74 21.2302 10.15L19.5502 18.9C19.1602 20.93 18.5102 22.75 14.8902 22.75Z"
                                fill="#0C0C0C" />
                        </svg>

                        <div className='sub-heading-content'>
                            <div className="sub-heading">Orders</div>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.7397 16.2799C10.5497 16.2799 10.3597 16.2099 10.2097 16.0599C9.91969 15.7699 9.91969 15.2899 10.2097 14.9999L13.2097 11.9999L10.2097 8.99991C9.91969 8.70991 9.91969 8.22991 10.2097 7.93991C10.4997 7.64991 10.9797 7.64991 11.2697 7.93991L14.7997 11.4699C15.0897 11.7599 15.0897 12.2399 14.7997 12.5299L11.2697 16.0599C11.1197 16.2099 10.9297 16.2799 10.7397 16.2799Z" fill="#B4B4B4" />
                            </svg>
                        </div>
                    </div>

                    <div className={`account-content ${activeTab === 'wishlist' ? 'active-tab' : ''}`}
                        onClick={() => setActiveTab("wishlist")}>

                        <svg className="account-content-svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M12 21.6501C11.69 21.6501 11.39 21.6101 11.14 21.5201C7.32 20.2101 1.25 15.5601 1.25 8.6901C1.25 5.1901 4.08 2.3501 7.56 2.3501C9.25 2.3501 10.83 3.0101 12 4.1901C13.17 3.0101 14.75 2.3501 16.44 2.3501C19.92 2.3501 22.75 5.2001 22.75 8.6901C22.75 15.5701 16.68 20.2101 12.86 21.5201C12.61 21.6101 12.31 21.6501 12 21.6501ZM7.56 3.8501C4.91 3.8501 2.75 6.0201 2.75 8.6901C2.75 15.5201 9.32 19.3201 11.63 20.1101C11.81 20.1701 12.2 20.1701 12.38 20.1101C14.68 19.3201 21.26 15.5301 21.26 8.6901C21.26 6.0201 19.1 3.8501 16.45 3.8501C14.93 3.8501 13.52 4.5601 12.61 5.7901C12.33 6.1701 11.69 6.1701 11.41 5.7901C10.48 4.5501 9.08 3.8501 7.56 3.8501Z"
                                fill="#0C0C0C" />
                        </svg>

                        <div className='sub-heading-content'>
                            <div className="sub-heading">Wish list</div>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.7397 16.2799C10.5497 16.2799 10.3597 16.2099 10.2097 16.0599C9.91969 15.7699 9.91969 15.2899 10.2097 14.9999L13.2097 11.9999L10.2097 8.99991C9.91969 8.70991 9.91969 8.22991 10.2097 7.93991C10.4997 7.64991 10.9797 7.64991 11.2697 7.93991L14.7997 11.4699C15.0897 11.7599 15.0897 12.2399 14.7997 12.5299L11.2697 16.0599C11.1197 16.2099 10.9297 16.2799 10.7397 16.2799Z" fill="#B4B4B4" />
                            </svg>
                        </div>
                    </div>

                    <div className={`account-content ${activeTab === 'discount' ? 'active-tab' : ''}`}
                        onClick={() => setActiveTab("discount")}>

                        <svg className="account-content-svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M15.9702 22.75H7.97021C4.55021 22.75 3.22021 21.42 3.22021 18V10C3.22021 9.59 3.56021 9.25 3.97021 9.25H19.9702C20.3802 9.25 20.7202 9.59 20.7202 10V18C20.7202 21.42 19.3902 22.75 15.9702 22.75ZM4.72021 10.75V18C4.72021 20.58 5.39021 21.25 7.97021 21.25H15.9702C18.5502 21.25 19.2202 20.58 19.2202 18V10.75H4.72021Z"
                                fill="#0C0C0C" />
                            <path
                                d="M19.5 10.75H4.5C2.75 10.75 1.75 9.75 1.75 8V7C1.75 5.25 2.75 4.25 4.5 4.25H19.5C21.2 4.25 22.25 5.3 22.25 7V8C22.25 9.7 21.2 10.75 19.5 10.75ZM4.5 5.75C3.59 5.75 3.25 6.09 3.25 7V8C3.25 8.91 3.59 9.25 4.5 9.25H19.5C20.38 9.25 20.75 8.88 20.75 8V7C20.75 6.12 20.38 5.75 19.5 5.75H4.5Z"
                                fill="#0C0C0C" />
                            <path
                                d="M11.6401 5.74988H6.12009C5.91009 5.74988 5.71009 5.65988 5.57009 5.50988C4.96009 4.83988 4.98009 3.80988 5.62009 3.16988L7.04009 1.74988C7.70009 1.08988 8.79009 1.08988 9.45009 1.74988L12.1701 4.46988C12.3801 4.67988 12.4501 5.00988 12.3301 5.28988C12.2201 5.56988 11.9501 5.74988 11.6401 5.74988ZM6.67009 4.24988H9.84009L8.39009 2.80988C8.31009 2.72988 8.18009 2.72988 8.10009 2.80988L6.68009 4.22988C6.68009 4.23988 6.67009 4.23988 6.67009 4.24988Z"
                                fill="#0C0C0C" />
                            <path
                                d="M17.8701 5.74988H12.3501C12.0501 5.74988 11.7701 5.56988 11.6601 5.28988C11.5401 5.00988 11.6101 4.68988 11.8201 4.46988L14.5401 1.74988C15.2001 1.08988 16.2901 1.08988 16.9501 1.74988L18.3701 3.16988C19.0101 3.80988 19.0401 4.83988 18.4201 5.50988C18.2801 5.65988 18.0801 5.74988 17.8701 5.74988ZM14.1701 4.24988H17.3401C17.3301 4.23988 17.3301 4.23988 17.3201 4.22988L15.9001 2.80988C15.8201 2.72988 15.6901 2.72988 15.6101 2.80988L14.1701 4.24988Z"
                                fill="#0C0C0C" />
                            <path
                                d="M9.93994 16.9C9.65994 16.9 9.36994 16.83 9.10994 16.69C8.53994 16.38 8.18994 15.79 8.18994 15.15V10C8.18994 9.59 8.52994 9.25 8.93994 9.25H14.9799C15.3899 9.25 15.7299 9.59 15.7299 10V15.13C15.7299 15.78 15.3799 16.37 14.8099 16.67C14.2399 16.98 13.5499 16.94 13.0099 16.58L12.1199 15.98C12.0399 15.92 11.9299 15.92 11.8399 15.98L10.8999 16.6C10.6099 16.8 10.2699 16.9 9.93994 16.9ZM9.68994 10.75V15.14C9.68994 15.27 9.76994 15.33 9.81994 15.36C9.86994 15.39 9.96994 15.42 10.0799 15.35L11.0199 14.73C11.6099 14.34 12.3699 14.34 12.9499 14.73L13.8399 15.33C13.9499 15.4 14.0499 15.37 14.0999 15.34C14.1499 15.31 14.2299 15.25 14.2299 15.12V10.74H9.68994V10.75Z"
                                fill="#0C0C0C" />
                        </svg>

                        <div className='sub-heading-content'>
                            <div className="sub-heading">Discounts</div>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.7397 16.2799C10.5497 16.2799 10.3597 16.2099 10.2097 16.0599C9.91969 15.7699 9.91969 15.2899 10.2097 14.9999L13.2097 11.9999L10.2097 8.99991C9.91969 8.70991 9.91969 8.22991 10.2097 7.93991C10.4997 7.64991 10.9797 7.64991 11.2697 7.93991L14.7997 11.4699C15.0897 11.7599 15.0897 12.2399 14.7997 12.5299L11.2697 16.0599C11.1197 16.2099 10.9297 16.2799 10.7397 16.2799Z" fill="#B4B4B4" />
                            </svg>
                        </div>
                    </div>

                    <div className={`account-content ${activeTab === 'security' ? 'active-tab' : ''}`}
                        onClick={() => setActiveTab("security")}>

                        <svg className="account-content-svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M11.9998 22.7501C11.7598 22.7501 11.5199 22.7201 11.2899 22.6501C6.09986 21.2201 2.33984 16.37 2.33984 11.11V6.72003C2.33984 5.60003 3.14985 4.39008 4.18985 3.96008L9.75986 1.68005C11.2099 1.09005 12.7998 1.09005 14.2398 1.68005L19.8098 3.96008C20.8498 4.39008 21.6599 5.60003 21.6599 6.72003V11.11C21.6599 16.36 17.8898 21.2101 12.7098 22.6501C12.4798 22.7201 12.2398 22.7501 11.9998 22.7501ZM11.9998 2.75006C11.4298 2.75006 10.8698 2.86007 10.3298 3.08007L4.75986 5.36004C4.27986 5.56004 3.83984 6.21004 3.83984 6.73004V11.1201C3.83984 15.7101 7.13985 19.9501 11.6898 21.2101C11.8898 21.2701 12.1098 21.2701 12.3098 21.2101C16.8598 19.9501 20.1599 15.7101 20.1599 11.1201V6.73004C20.1599 6.21004 19.7198 5.56004 19.2398 5.36004L13.6699 3.08007C13.1299 2.86007 12.5698 2.75006 11.9998 2.75006Z"
                                fill="#0C0C0C" />
                            <path
                                d="M12 13.25C10.48 13.25 9.25 12.02 9.25 10.5C9.25 8.98 10.48 7.75 12 7.75C13.52 7.75 14.75 8.98 14.75 10.5C14.75 12.02 13.52 13.25 12 13.25ZM12 9.25C11.31 9.25 10.75 9.81 10.75 10.5C10.75 11.19 11.31 11.75 12 11.75C12.69 11.75 13.25 11.19 13.25 10.5C13.25 9.81 12.69 9.25 12 9.25Z"
                                fill="#0C0C0C" />
                            <path
                                d="M12 16.25C11.59 16.25 11.25 15.91 11.25 15.5V12.5C11.25 12.09 11.59 11.75 12 11.75C12.41 11.75 12.75 12.09 12.75 12.5V15.5C12.75 15.91 12.41 16.25 12 16.25Z"
                                fill="#0C0C0C" />
                        </svg>

                        <div className='sub-heading-content'>
                            <div className="sub-heading">Security & access</div>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.7397 16.2799C10.5497 16.2799 10.3597 16.2099 10.2097 16.0599C9.91969 15.7699 9.91969 15.2899 10.2097 14.9999L13.2097 11.9999L10.2097 8.99991C9.91969 8.70991 9.91969 8.22991 10.2097 7.93991C10.4997 7.64991 10.9797 7.64991 11.2697 7.93991L14.7997 11.4699C15.0897 11.7599 15.0897 12.2399 14.7997 12.5299L11.2697 16.0599C11.1197 16.2099 10.9297 16.2799 10.7397 16.2799Z" fill="#B4B4B4" />
                            </svg>
                        </div>
                    </div>

                    <div className="account-content" onClick={handleContactUsClick}>

                        <svg className="account-content-svg" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 22.75C11.3 22.75 10.59 22.48 10.06 21.95L8.35001 20.26C7.92001 19.84 7.35999 19.61 6.75999 19.61H6C3.93 19.61 2.25 17.94 2.25 15.89V4.97998C2.25 2.92998 3.93 1.26001 6 1.26001H18C20.07 1.26001 21.75 2.92998 21.75 4.97998V15.89C21.75 17.94 20.07 19.61 18 19.61H17.24C16.64 19.61 16.07 19.84 15.65 20.26L13.94 21.95C13.41 22.48 12.7 22.75 12 22.75ZM6 2.75C4.76 2.75 3.75 3.74997 3.75 4.96997V15.88C3.75 17.11 4.76 18.1 6 18.1H6.75999C7.75999 18.1 8.7 18.4899 9.41 19.1899L11.12 20.88C11.61 21.36 12.4 21.36 12.89 20.88L14.6 19.1899C15.31 18.4899 16.25 18.1 17.25 18.1H18C19.24 18.1 20.25 17.1 20.25 15.88V4.96997C20.25 3.73997 19.24 2.75 18 2.75H6Z" fill="#0C0C0C" />
                            <path d="M10.3802 14.51H7.70023C7.26023 14.51 6.85023 14.2999 6.59023 13.9399C6.34023 13.5999 6.28022 13.18 6.40022 12.78C6.75022 11.71 7.61022 11.13 8.37022 10.61C9.17022 10.07 9.62022 9.73002 9.62022 9.15002C9.62022 8.63002 9.20022 8.20996 8.68022 8.20996C8.16022 8.20996 7.74023 8.63002 7.74023 9.15002C7.74023 9.56002 7.40023 9.90002 6.99023 9.90002C6.58023 9.90002 6.24023 9.56002 6.24023 9.15002C6.24023 7.81002 7.33022 6.70996 8.68022 6.70996C10.0302 6.70996 11.1202 7.80002 11.1202 9.15002C11.1202 10.56 10.0602 11.28 9.21024 11.86C8.68024 12.22 8.18022 12.56 7.93022 13.01H10.3702C10.7802 13.01 11.1202 13.35 11.1202 13.76C11.1202 14.17 10.7902 14.51 10.3802 14.51Z" fill="#0C0C0C" />
                            <path d="M16.0399 14.5099C15.6299 14.5099 15.2899 14.1699 15.2899 13.7599V13.0699H13.3299C13.3299 13.0699 13.3299 13.0699 13.3199 13.0699C12.8299 13.0699 12.3799 12.81 12.1299 12.39C11.8799 11.96 11.8799 11.4299 12.1299 11.0099C12.8099 9.83995 13.5999 8.50991 14.3199 7.34991C14.6399 6.83991 15.2499 6.60996 15.8199 6.76996C16.3899 6.93996 16.7899 7.45999 16.7799 8.05999V11.5799H16.9999C17.4099 11.5799 17.7499 11.9199 17.7499 12.3299C17.7499 12.7399 17.4099 13.0799 16.9999 13.0799H16.7899V13.77C16.7899 14.18 16.4599 14.5099 16.0399 14.5099ZM15.2899 8.63995C14.6999 9.59995 14.0899 10.6299 13.5399 11.5699H15.2899V8.63995Z" fill="#0C0C0C" />
                        </svg>

                        <div className='sub-heading-content'>
                            <div className="sub-heading">Contact Us</div>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.7397 16.2799C10.5497 16.2799 10.3597 16.2099 10.2097 16.0599C9.91969 15.7699 9.91969 15.2899 10.2097 14.9999L13.2097 11.9999L10.2097 8.99991C9.91969 8.70991 9.91969 8.22991 10.2097 7.93991C10.4997 7.64991 10.9797 7.64991 11.2697 7.93991L14.7997 11.4699C15.0897 11.7599 15.0897 12.2399 14.7997 12.5299L11.2697 16.0599C11.1197 16.2099 10.9297 16.2799 10.7397 16.2799Z" fill="#B4B4B4" />
                            </svg>
                        </div>
                    </div>

                    <div className="account-content">

                        <svg className="account-content-svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M15.24 22.27H15.11C10.67 22.27 8.53002 20.52 8.16002 16.6C8.12002 16.19 8.42002 15.82 8.84002 15.78C9.24002 15.74 9.62002 16.05 9.66002 16.46C9.95002 19.6 11.43 20.77 15.12 20.77H15.25C19.32 20.77 20.76 19.33 20.76 15.26V8.73998C20.76 4.66998 19.32 3.22998 15.25 3.22998H15.12C11.41 3.22998 9.93002 4.41998 9.66002 7.61998C9.61002 8.02998 9.26002 8.33998 8.84002 8.29998C8.42002 8.26998 8.12001 7.89998 8.15001 7.48998C8.49001 3.50998 10.64 1.72998 15.11 1.72998H15.24C20.15 1.72998 22.25 3.82998 22.25 8.73998V15.26C22.25 20.17 20.15 22.27 15.24 22.27Z"
                                fill="#444444" />
                            <path
                                d="M15.0001 12.75H3.62012C3.21012 12.75 2.87012 12.41 2.87012 12C2.87012 11.59 3.21012 11.25 3.62012 11.25H15.0001C15.4101 11.25 15.7501 11.59 15.7501 12C15.7501 12.41 15.4101 12.75 15.0001 12.75Z"
                                fill="#444444" />
                            <path
                                d="M5.84994 16.1001C5.65994 16.1001 5.46994 16.0301 5.31994 15.8801L1.96994 12.5301C1.67994 12.2401 1.67994 11.7601 1.96994 11.4701L5.31994 8.12009C5.60994 7.83009 6.08994 7.83009 6.37994 8.12009C6.66994 8.41009 6.66994 8.89009 6.37994 9.18009L3.55994 12.0001L6.37994 14.8201C6.66994 15.1101 6.66994 15.5901 6.37994 15.8801C6.23994 16.0301 6.03994 16.1001 5.84994 16.1001Z"
                                fill="#444444" />
                        </svg>

                        <div className='sub-heading-content'>
                            <div className="sub-heading" style={{ color: "red" }} onClick={() => setShowModel(true)}>Log out</div>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.7397 16.2799C10.5497 16.2799 10.3597 16.2099 10.2097 16.0599C9.91969 15.7699 9.91969 15.2899 10.2097 14.9999L13.2097 11.9999L10.2097 8.99991C9.91969 8.70991 9.91969 8.22991 10.2097 7.93991C10.4997 7.64991 10.9797 7.64991 11.2697 7.93991L14.7997 11.4699C15.0897 11.7599 15.0897 12.2399 14.7997 12.5299L11.2697 16.0599C11.1197 16.2099 10.9297 16.2799 10.7397 16.2799Z" fill="#B4B4B4" />
                            </svg>
                        </div>
                    </div>
                </div>

                {renderActiveContent()}
            </div>
            {showModel &&
                <LogoutModel
                    onClose={closeModel}
                    onShowLogin={() => setShowLoginModel(true)}
                    onLogout={() => setShowLoginModel(true)}
                />
            }
            {showLoginModel && <LoginModel onClose={handlecloseLogin} />}
            <Footer />
        </>
    )
}

export default Profile;